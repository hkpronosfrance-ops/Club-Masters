...
const playerName = (p?: EnginePlayer, fallback = "Un joueur") => p ? `${p.first_name} ${p.last_name}` : fallback;
...
function selectXI(club: EngineClub) {
  ...
  if (selected?.length === 11 && selected.some(player => player.position === "GK")) return [...selected];
  ...
  if (!xi.some(player => player.position === "GK")) {
    const goalkeeper = club.players.filter(player => player.position === "GK").sort((a,b)=>effective(b)-effective(a))[0];
    if (goalkeeper) {
      const replacementIndex = xi.findIndex(player => player.position !== "GK");
      if (replacementIndex >= 0) xi[replacementIndex] = goalkeeper;
    }
  }
...
export function simulateMatch(home: EngineClub, away: EngineClub, options: { weather?: "sunny"|"rain"|"cold"; neutralVenue?: boolean } = {}): MatchResult {
  ...
  const events: MatchEvent[]=[{minute:1,type:"kickoff",team:"home",playerName:"Arbitre",commentary:"Coup d’envoi de la rencontre."}]; const tacticalReactions:TacticalReaction[]=[]; const stats={home:emptyStats(),away:emptyStats()};
  ...
  for(let minute=1;minute<=90;minute++){
    if(minute===46)events.push({minute:45,type:"half_time",team:"home",playerName:"Arbitre",commentary:`Mi-temps : ${home.name} ${homeScore}-${awayScore} ${away.name}.`});
    ...
    if(Math.random()<.018*(2-attack.discipline)*(weather==="rain"?1.18:1)){oppStats.fouls++;const offender=weighted(defenders.filter(p=>p.position!=="GK"),"defending"),roll=Math.random();if(roll<.035){oppStats.redCards++;events.push({minute,type:"red",team:homeBall?"away":"home",playerName:playerName(offender,"Un défenseur"),commentary:`${playerName(offender,"Un défenseur")} est expulsé.`});}else if(roll<.3){oppStats.yellowCards++;events.push({minute,type:"yellow",team:homeBall?"away":"home",playerName:playerName(offender,"Un défenseur"),commentary:`Carton jaune pour ${playerName(offender,"un défenseur")}.`});}}
    if(Math.random()>=clamp(attack.attack/Math.max(45,defense.defense)*.034*attack.tempo,.012,.095))continue;
    if(Math.random()<.08){teamStats.offsides++;const runner=weighted(attack.attackers.length?attack.attackers:attackers,"pace");events.push({minute,type:"offside",team,playerName:playerName(runner,"Un attaquant"),commentary:`${playerName(runner,"Un attaquant")} est signalé hors-jeu.`});continue;}
    const shooter=weighted(attack.attackers.length?attack.attackers:attackers,"shooting") ?? attackers[0],creator=weighted(attack.mids.length?attack.mids:attackers,"passing") ?? attackers[0],keeper=defenders.find(p=>p.position==="GK") ?? defenders[0];
    ...
    if(penalty)events.push({minute,type:"penalty",team,playerName:playerName(shooter,"Un attaquant"),commentary:`Penalty accordé à ${club.name} !`,xg}); else if(Math.random()<.13){teamStats.corners++;events.push({minute,type:"corner",team,playerName:playerName(creator,"Un milieu"),commentary:`Corner obtenu par ${club.name}.`,xg});}
    if(Math.random()>=clamp(.34+Number(shooter?.shooting??50)/220,.38,.78)){events.push({minute,type:"chance_missed",team,playerName:playerName(shooter,"Un attaquant"),secondaryPlayerName:playerName(creator,"Un milieu"),commentary:`${playerName(shooter,"Un attaquant")} manque le cadre.`,xg});continue;}
    ...
    if(Math.random()<goalProb){if(homeBall)homeScore++;else awayScore++;events.push({minute,type:"goal",team,playerName:playerName(shooter,"Un attaquant"),secondaryPlayerName:playerName(creator,"Un milieu"),commentary:`BUT ! ${playerName(shooter,"Un attaquant")} conclut l’action de ${club.name}.`,xg});}else events.push({minute,type:"save",team:homeBall?"away":"home",playerName:playerName(keeper,"Le gardien"),secondaryPlayerName:playerName(shooter,"Un attaquant"),commentary:`${playerName(keeper,"Le gardien")} repousse la tentative.`,xg});
  }
  ...
  events.push({minute:90,type:"full_time",team:"home",playerName:"Arbitre",commentary:`Score final : ${home.name} ${homeScore}-${awayScore} ${away.name}.`});events.sort((a,b)=>a.minute-b.minute);
