...
import { updatePlayerSeasonStats } from "@/lib/playerStats";
...
export async function POST(request: Request) {
  ...
  const [{ data: myClub }, { data: opponent }, { data: myPlayers }, { data: opponentPlayers }] = await Promise.all([
    admin.from("clubs").select("*").eq("id", profile.club_id).single(),
    admin.from("clubs").select("*").eq("id", opponentId).single(),
    admin.from("players").select("*").eq("club_id", profile.club_id),
    admin.from("players").select("*").eq("club_id", opponentId),
  ]);
  ...
  await updatePlayerSeasonStats({
    ...
    homeClubId: home.id,
    awayClubId: away.id,
    homePlayers: homeIsMe ? ownedPlayers : opponentAvailable,
    awayPlayers: homeIsMe ? opponentAvailable : ownedPlayers,
    homeStarterIds: homeIsMe ? starterIds : opponentStarterIds,
    awayStarterIds: homeIsMe ? opponentStarterIds : starterIds,
  ...
  const { data: attendance } = homeIsMe ? await admin.from("match_attendance").select("*").eq("match_id", savedMatch.id).eq("club_id", myClub.id).maybeSingle() : { data: null };
  const stadiumRevenue = homeIsMe && attendance ? Number(attendance.ticket_revenue ?? 0) + Number(attendance.vip_revenue ?? 0) + Number(attendance.catering_revenue ?? 0) + Number(attendance.merchandise_revenue ?? 0) : 0;
  const balanceAfterMatch = Number(myClub.balance ?? 0) + matchBonus + stadiumRevenue;
  await admin.from("clubs").update({ wins: Number(myClub.wins ?? 0) + (outcome === "win" ? 1 : 0), draws: Number(myClub.draws ?? 0) + (outcome === "draw" ? 1 : 0), losses: Number(myClub.losses ?? 0) + (outcome === "loss" ? 1 : 0), balance: balanceAfterMatch }).eq("id", myClub.id);

  const financeRows = [
    { club_id: myClub.id, match_id: savedMatch.id, category: "match_bonus", amount: matchBonus, description: `Prime de match contre ${opponent.name}` },
    ...(homeIsMe && attendance ? [
      { club_id: myClub.id, match_id: savedMatch.id, category: "ticketing", amount: Number(attendance.ticket_revenue ?? 0), description: `Billetterie — ${attendance.attendance} spectateurs` },
      { club_id: myClub.id, match_id: savedMatch.id, category: "vip", amount: Number(attendance.vip_revenue ?? 0), description: "Revenus espaces VIP" },
      { club_id: myClub.id, match_id: savedMatch.id, category: "catering", amount: Number(attendance.catering_revenue ?? 0), description: "Restauration du stade" },
      { club_id: myClub.id, match_id: savedMatch.id, category: "merchandise", amount: Number(attendance.merchandise_revenue ?? 0), description: "Boutique et produits dérivés" },
    ] : []),
  ].filter((row) => row.amount !== 0);
  if (financeRows.length) await admin.from("club_finance_transactions").upsert(financeRows, { onConflict: "club_id,match_id,category" });
  ...
  for (const player of ownedPlayers) {
    ...
    const update: Record<string, unknown> = { fatigue: nextFatigue, form: Math.max(0, Math.min(100, Number(player.form ?? 50) + (played ? formDelta + Math.floor(Math.random() * 6 - 3) : 0))) };
    if (played && Math.random() < injuryRisk(nextFatigue) * medicalEffects.injuryMultiplier) {
      const baseDays = nextFatigue >= 90 ? 10 + Math.floor(Math.random() * 12) : 3 + Math.floor(Math.random() * 7);
      const days = Math.max(1, baseDays - Math.floor(staffLevels.doctor / 2));
      const types = ["Lésion musculaire", "Entorse", "Contusion"];
      const type = types[Math.floor(Math.random() * types.length)];
      update.injured_until = new Date(Date.now() + days * 86_400_000).toISOString(); update.injury_type = type;
      injuries.push({ id: player.id, name: fullName, type, days });
    }
  ...
  if (aiFixtures?.length) {
    const clubIds = [...new Set(aiFixtures.flatMap((item) => [item.home_club_id, item.away_club_id]))];
    const { data: clubs } = await admin.from("clubs").select("id,reputation").in("id", clubIds);
    const reps = new Map((clubs ?? []).map((club) => [club.id, club.reputation ?? 50]));
    for (const item of aiFixtures) {
      const [homeScore, awayScore] = simulateAiScore(reps.get(item.home_club_id) ?? 50, reps.get(item.away_club_id) ?? 50);
      await admin.from("league_fixtures").update({ home_score: homeScore, away_score: awayScore, played: true, played_at: new Date().toISOString() }).eq("id", item.id);
      await updateStanding(admin, season.id, item.home_club_id, homeScore, awayScore); await updateStanding(admin, season.id, item.away_club_id, awayScore, homeScore);
    }
  }
  const nextRound = season.current_round + 1;
  const seasonFinished = nextRound > season.total_rounds;
  ...
  return NextResponse.json({ result, matchId: savedMatch.id, homeIsMe, home: { id: home.id, name: home.name }, away: { id: away.id, name: away.name }, opponent: { id: opponent.id, name: opponent.name }, outcome, matchBonus, stadiumRevenue, ticketRevenue: Number(attendance?.ticket_revenue ?? 0), attendance: attendance ? { spectators: attendance.attendance, atmosphere: attendance.atmosphere, ticketRevenue: Number(attendance.ticket_revenue ?? 0), vipRevenue: Number(attendance.vip_revenue ?? 0), cateringRevenue: Number(attendance.catering_revenue ?? 0), merchandiseRevenue: Number(attendance.merchandise_revenue ?? 0), totalRevenue: stadiumRevenue } : null, staffImpact: { doctorLevel: staffLevels.doctor, fitnessCoachLevel: staffLevels.fitness_coach, injuryReductionPercent: Math.round((1 - medicalEffects.injuryMultiplier) * 100), fatigueGainReduction: medicalEffects.fatigueGainReduction, recoveryBonus: medicalEffects.recoveryBonus }, injuries, round: season.current_round, seasonFinished, board });
}
