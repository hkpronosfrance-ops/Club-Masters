...
export async function POST(request: Request) {
  ...
  const { data: seasonData } = await admin
    ...
    .select("*")
    .eq("status", "active")
    .eq("user_club_id", profile.club_id)
  ...
  if (!season.objective_code) {
    ...
    const { data: initialized } = await admin
      .from("seasons")
      .update({ user_club_id: myClub.id, objective_code: objective.code, objective_label: objective.label, target_position: objective.target })
      .eq("id", season.id)
      .select("*")
      .single();
  ...
  const opponentEngine = toEngineClub(opponent, opponentAvailable, opponentStarters.map((player: any) => player.id));
  ...
  const { data: savedMatch, error: matchError } = await admin
    .from("matches")
    .insert({
  ...
  const { data: attendance } = homeIsMe
    ? await admin.from("match_attendance").select("*").eq("match_id", savedMatch.id).eq("club_id", myClub.id).maybeSingle()
    : { data: null };

  const stadiumRevenue = homeIsMe && attendance
    ? Number(attendance.ticket_revenue ?? 0) + Number(attendance.vip_revenue ?? 0) + Number(attendance.catering_revenue ?? 0) + Number(attendance.merchandise_revenue ?? 0)
    : 0;
  const balanceAfterMatch = Number(myClub.balance ?? 0) + matchBonus + stadiumRevenue;

  await admin.from("clubs").update({
    wins: Number(myClub.wins ?? 0) + (outcome === "win" ? 1 : 0),
    draws: Number(myClub.draws ?? 0) + (outcome === "draw" ? 1 : 0),
    losses: Number(myClub.losses ?? 0) + (outcome === "loss" ? 1 : 0),
    balance: balanceAfterMatch,
  }).eq("id", myClub.id);
  ...
  if (aiFixtures?.length) {
    ...
    for (const item of aiFixtures) {
      ...
      await updateStanding(admin, season.id, item.home_club_id, homeScore, awayScore);
      await updateStanding(admin, season.id, item.away_club_id, awayScore, homeScore);
    }
  }

  const nextRound = season.current_round + 1;
  const seasonFinished = nextRound > season.total_rounds;
  await admin.from("seasons").update(seasonFinished
    ? { current_round: nextRound, status: "finished", finished_at: new Date().toISOString() }
  ...
  return NextResponse.json({
    ...
    matchBonus,
    stadiumRevenue,
    ticketRevenue: Number(attendance?.ticket_revenue ?? 0),
    attendance: attendance ? {
      spectators: attendance.attendance,
      atmosphere: attendance.atmosphere,
      ticketRevenue: Number(attendance.ticket_revenue ?? 0),
      vipRevenue: Number(attendance.vip_revenue ?? 0),
      cateringRevenue: Number(attendance.catering_revenue ?? 0),
