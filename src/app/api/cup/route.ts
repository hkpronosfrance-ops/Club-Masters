...
function strength(club: any) { return Math.max(35, Math.min(95, Number(club?.reputation ?? 50))); }
...
async function userContext() {
  ...
  return profile?.club_id ? { userId: user.id, clubId: profile.club_id } : null;
}
async function ensureCompetition() {
  ...
  await admin.from("cup_matches").insert(Array.from({ length: 4 }, (_, index) => ({ competition_id: competition.id, round: 1, match_order: index + 1, home_club_id: draw[index * 2].id, away_club_id: draw[index * 2 + 1].id })));
  return competition;
}
...
export async function GET() {
  ...
  try { return NextResponse.json(await cupPayload()); }
  catch (error: any) { return NextResponse.json({ error: error.message ?? "Impossible de charger la coupe." }, { status: 500 }); }
}
export async function POST() {
  const context = await userContext();
  if (!context) return NextResponse.json({ error: "Non authentifié ou aucun club." }, { status: 401 });
  ...
  try {
    ...
    let winnerId: string;
    if (homeScore === awayScore) { extraTime = true; if (Math.random() < 0.45) Math.random() < 0.5 ? homeScore++ : awayScore++; }
    if (homeScore === awayScore) {
      homePenalties = 3 + Math.floor(Math.random() * 3); awayPenalties = 3 + Math.floor(Math.random() * 3);
    ...
    } else winnerId = homeScore > awayScore ? pending.home_club_id : pending.away_club_id;
    ...
    if (userInMatch) {
      const opponent = pending.home_club_id === clubId ? pending.away : pending.home;
      const myClub = pending.home_club_id === clubId ? pending.home : pending.away;
      await recordManagerMatch(admin, { userId, clubId, outcome: winnerId === clubId ? "win" : "loss", opponentReputation: opponent?.reputation, clubReputation: myClub?.reputation });
      const { data: squad } = await admin.from("players").select("id,fatigue").eq("club_id", clubId);
      await Promise.all((squad ?? []).map((player: any) => admin.from("players").update({ fatigue: Math.min(100, Number(player.fatigue ?? 0) + 7) }).eq("id", player.id)));
    }
    ...
    if (roundMatches.filter((match: any) => !match.played && match.id !== pending.id).length === 0) {
      ...
      if (pending.round < 3) {
        ...
        await admin.from("cup_matches").insert(Array.from({ length: winners.length / 2 }, (_, index) => ({ competition_id: competition.id, round: nextRound, match_order: index + 1, home_club_id: winners[index * 2], away_club_id: winners[index * 2 + 1] })));
        ...
        const seasonLabel = season?.name ?? "Saison actuelle";
        ...
        await admin.from("club_trophies").upsert({ club_id: winnerId, competition_name: competition.name, season_name: seasonLabel });
        if (winnerId === clubId) await awardManagerTrophy(admin, { userId, clubId, trophyType: "national_cup", trophyName: competition.name, season: seasonLabel, reputationBonus: 6 });
      }
    }
    return NextResponse.json(await cupPayload());
  } catch (error: any) { return NextResponse.json({ error: error.message ?? "La simulation a échoué." }, { status: 500 }); }
}
