...
import { awardManagerTrophy, recordManagerMatch } from "@/lib/managerCareer";

const CONFIG = {
  champions_league: { name: "Ligue des Champions", prizes: { 1: 1_000_000, 2: 2_500_000, 3: 7_500_000 }, reputation: 7, managerReputation: 12 },
  europa_league: { name: "Ligue Europa", prizes: { 1: 500_000, 2: 1_250_000, 3: 3_500_000 }, reputation: 4, managerReputation: 8 },
} as const;
...
function simulate(home: any, away: any) {
  const homePower = Number(home.reputation ?? 50) + 3; const awayPower = Number(away.reputation ?? 50);
  ...
  let extraTime = false; let homePenalties: number | null = null; let awayPenalties: number | null = null;
  if (homeScore === awayScore) { extraTime = true; if (Math.random() < 0.5) Math.random() < 0.5 ? homeScore++ : awayScore++; }
  if (homeScore === awayScore) { homePenalties = 3 + Math.floor(Math.random() * 3); awayPenalties = 3 + Math.floor(Math.random() * 3); while (homePenalties === awayPenalties) Math.random() < 0.5 ? homePenalties++ : awayPenalties++; }
  const winnerId = homeScore !== awayScore ? (homeScore > awayScore ? home.id : away.id) : ((homePenalties ?? 0) > (awayPenalties ?? 0) ? home.id : away.id);
...
async function userContext() {
  ...
  const admin = createAdminClient(); const { data } = await admin.from("profiles").select("club_id").eq("id", user.id).single();
  return data?.club_id ? { userId: user.id, clubId: data.club_id } : null;
}
async function ensureCompetition(code: CompetitionCode) {
  ...
  await admin.from("european_matches").insert(Array.from({ length: 4 }, (_, index) => ({ competition_id: competition.id, round: 1, match_order: index + 1, home_club_id: draw[index * 2].id, away_club_id: draw[index * 2 + 1].id })));
  return competition;
}
async function payload(code: CompetitionCode) {
  const admin = createAdminClient(); const competition = await ensureCompetition(code);
...
export async function GET(req: Request) {
  if (!await userContext()) return NextResponse.json({ error: "Non authentifié ou aucun club." }, { status: 401 });
  ...
  try { return NextResponse.json(await payload(code)); } catch (error: any) { return NextResponse.json({ error: error.message ?? "Chargement impossible." }, { status: 500 }); }
}
export async function POST(req: Request) {
  const context = await userContext(); if (!context) return NextResponse.json({ error: "Non authentifié ou aucun club." }, { status: 401 });
  const { userId, clubId } = context;
  const code = (new URL(req.url).searchParams.get("code") as CompetitionCode) || "champions_league";
  if (!CONFIG[code]) return NextResponse.json({ error: "Compétition invalide." }, { status: 400 });
  const admin = createAdminClient();
  try {
    const { competition, matches } = await payload(code); const pending = matches.find((match: any) => !match.played);
    ...
    await admin.from("european_matches").update({ home_score: result.homeScore, away_score: result.awayScore, extra_time: result.extraTime, home_penalties: result.homePenalties, away_penalties: result.awayPenalties, winner_club_id: result.winnerId, played: true, played_at: new Date().toISOString() }).eq("id", pending.id);
    const { data: winner } = await admin.from("clubs").select("balance,reputation").eq("id", result.winnerId).single();
    await admin.from("clubs").update({ balance: Number(winner?.balance ?? 0) + CONFIG[code].prizes[pending.round as 1 | 2 | 3], reputation: Math.min(100, Number(winner?.reputation ?? 50) + pending.round) }).eq("id", result.winnerId);
    const userInMatch = [pending.home_club_id, pending.away_club_id].includes(clubId);
    if (userInMatch) {
      const opponent = pending.home_club_id === clubId ? pending.away : pending.home; const myClub = pending.home_club_id === clubId ? pending.home : pending.away;
      await recordManagerMatch(admin, { userId, clubId, outcome: result.winnerId === clubId ? "win" : "loss", opponentReputation: opponent?.reputation, clubReputation: myClub?.reputation });
    ...
    if (roundMatches.filter((match: any) => !match.played && match.id !== pending.id).length === 0) {
      ...
      if (pending.round < 3) {
        ...
        const { data: season } = await admin.from("seasons").select("name").eq("id", competition.season_id).maybeSingle(); const seasonLabel = season?.name ?? "Saison actuelle";
        ...
        await admin.from("club_trophies").insert({ club_id: result.winnerId, competition_name: CONFIG[code].name, season_name: seasonLabel });
        if (result.winnerId === clubId) await awardManagerTrophy(admin, { userId, clubId, trophyType: code, trophyName: CONFIG[code].name, season: seasonLabel, reputationBonus: CONFIG[code].managerReputation });
      }
    }
    return NextResponse.json(await payload(code));
  } catch (error: any) { return NextResponse.json({ error: error.message ?? "Simulation impossible." }, { status: 500 }); }
}
