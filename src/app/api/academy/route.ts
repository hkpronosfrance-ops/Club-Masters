...
import { academyStaffEffects, loadStaffLevels } from "@/lib/staffEffects";
...
async function academyPayload(club: any) {
  const admin = createAdminClient();
  const [{ data: players }, staff] = await Promise.all([
    admin.from("academy_players").select("*").eq("club_id", club.id).eq("status", "academy").order("potential", { ascending: false }),
    loadStaffLevels(admin, club.id),
  ]);
  return { club, players: players ?? [], upgradeCost: academyUpgradeCost(club.academy_level ?? 1), staff, staffEffects: academyStaffEffects(staff) };
}
...
export async function POST(request: NextRequest) {
  ...
  const staff = await loadStaffLevels(admin, club.id);
  const effects = academyStaffEffects(staff);
  ...
  if (action === "intake") {
    ...
    const amount = Math.min(available, academyIntakeSize(club.academy_level ?? 1) + effects.intakeBonus);
    const rows = Array.from({ length: amount }, () => {
      const generated = generateAcademyPlayer(club.academy_level ?? 1);
      const boostedPotential = Math.min(99, generated.potential + effects.potentialBonus);
      const boostedOverall = Math.min(boostedPotential, generated.overall + (Math.random() < effects.overallBonusChance ? 1 : 0));
      return { ...generated, overall: boostedOverall, potential: boostedPotential, club_id: club.id };
    });
    const { error } = await admin.from("academy_players").insert(rows);
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    const nextDate = new Date();
    nextDate.setDate(nextDate.getDate() + effects.cooldownDays);
  ...
  if (action === "promote") {
    ...
    const { data: player, error } = await admin.from("players").insert({
      ...
      pace: youth.pace,
      shooting: youth.shooting,
      passing: youth.passing,
      defending: youth.defending,
      physical: youth.physical,
      morale: 75,
      fatigue: 0,
      form: 50,
      value: Math.round(Math.pow(youth.overall, 3.2) * 1.8),
      wage: Math.max(800, youth.overall * 45),
