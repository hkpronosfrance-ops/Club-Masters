...
export default function TacticsPage() {
  ...
  async function playMatch() {
    if (!canPlay) return;
    setSimulating(true); setError(null); setMatchResult(null);
    try {
      const response = await fetch("/api/match/simulate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ starterIds }) });
    ...
    } catch (caught: any) { setError(caught.message ?? "La rencontre n’a pas pu être lancée."); }
  ...
  return <div className="min-h-screen pitch-bg pb-28 md:pb-10"><Nav /><main className="mx-auto max-w-6xl px-4 py-6 md:px-6 md:py-10">
    ...
    <div className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
      <section className="rounded-2xl border border-white/10 bg-pitch-900/85 p-4 md:p-5"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] uppercase tracking-[0.18em] text-muted">Feuille de match</p><h2 className="mt-1 font-display text-2xl">Les 11 titulaires</h2></div><button onClick={autoSelect} className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs text-zinc-200">Sélection auto</button></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{players.map((player) => { const selected = starterIds.includes(player.id); return <button key={player.id} onClick={() => toggleStarter(player.id)} className={`flex items-center gap-3 rounded-xl border p-3 text-left transition ${selected ? "border-carmine/60 bg-carmine/10" : "border-white/8 bg-white/[0.03] hover:border-white/20"}`}><div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl font-display text-lg ${selected ? "bg-carmine text-white" : "bg-white/5 text-zinc-200"}`}>{player.overall}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{player.first_name} {player.last_name}</p><p className="mt-1 text-[10px] text-muted">{player.position} · Forme {player.form} · Fraîcheur {100 - player.fatigue}</p></div><span className={`text-lg ${selected ? "text-carmine-light" : "text-muted"}`}>{selected ? "✓" : "+"}</span></button>; })}</div></section>
      <section className="space-y-5"><div className="rounded-2xl border border-white/10 bg-pitch-900/85 p-5"><p className="text-[10px] uppercase tracking-[0.18em] text-muted">Organisation</p><h2 className="mt-1 font-display text-2xl">Formation</h2><div className="mt-4 grid grid-cols-2 gap-2">{FORMATIONS.map((formation) => <button key={formation} onClick={() => updateClub({ formation })} className={`rounded-xl border px-3 py-3 font-mono text-sm ${club.formation === formation ? "border-carmine bg-carmine text-white" : "border-white/10 bg-white/5 text-muted"}`}>{formation}</button>)}</div><p className="mt-6 text-[10px] uppercase tracking-[0.18em] text-muted">Style de jeu</p><div className="mt-3 grid grid-cols-2 gap-2">{STYLES.map((style) => <button key={style.value} onClick={() => updateClub({ tactic_style: style.value })} className={`rounded-xl border px-3 py-3 text-sm ${club.tactic_style === style.value ? "border-carmine/50 bg-carmine/10 text-white" : "border-white/10 bg-white/5 text-muted"}`}>{style.label}</button>)}</div><div className="mt-6 flex items-center justify-between"><span className="text-sm text-muted">Mentalité</span><span className="font-mono text-sm">{club.mentality}</span></div><input type="range" min={0} max={100} value={club.mentality} onChange={(event) => updateClub({ mentality: Number(event.target.value) })} className="mt-3 w-full accent-carmine" /></div>
    ...
    {matchResult && <MatchSummary data={matchResult} />}
  </main></div>;
}
...
function MatchSummary({ data }: { data: any }) {
  ...
  return <section className="mt-5 overflow-hidden rounded-3xl border border-white/10 bg-pitch-900/90"><div className="bg-black/15 px-5 py-6 text-center"><p className="text-[10px] uppercase tracking-[0.2em] text-muted">Score final</p><div className="mt-3 grid grid-cols-[1fr_auto_1fr] items-center gap-3"><span className="font-display text-lg">{data.home.name}</span><span className="font-display text-4xl font-bold md:text-6xl">{data.result.homeScore} - {data.result.awayScore}</span><span className="font-display text-lg">{data.away.name}</span></div><p className="mt-3 font-mono text-xs text-gold">Prime {money(data.matchBonus)} · Recettes stade {money(data.stadiumRevenue)}</p></div>
    ...
    </div><div className="border-t border-white/10 p-5"><h3 className="font-display text-xl">Temps forts</h3><div className="mt-3 space-y-2">{data.result.events.map((event: any, index: number) => <div key={`${event.minute}-${index}`} className="flex items-center gap-3 rounded-xl bg-white/5 px-3 py-3 text-sm"><span className="w-10 font-mono text-muted">{event.minute}&apos;</span><span>{event.type === "substitution" ? "🔄" : "⚽"}</span><span>{event.playerName}</span></div>)}{!data.result.events.length && <p className="rounded-xl bg-white/5 p-4 text-center text-sm text-muted">Aucun événement majeur dans cette rencontre.</p>}</div></div></section>;
}
