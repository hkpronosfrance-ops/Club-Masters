  ...
  return (
    <div className="min-h-screen pitch-bg pb-28 md:pb-10">
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 md:px-6 md:py-10">
        <section className="relative mb-6 overflow-hidden rounded-2xl border border-white/10 bg-pitch-900/90 p-5 shadow-2xl shadow-black/20 md:p-7">
          ...
          <div className="relative flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="mb-2 font-mono text-[11px] uppercase tracking-[0.28em] text-carmine-light">Cellule de recrutement</p>
              <h1 className="font-display text-3xl font-semibold tracking-tight md:text-4xl">Mercato mondial</h1>
            ...
            <div className="rounded-xl border border-gold/20 bg-gold/5 px-5 py-4">
              <p className="text-[10px] uppercase tracking-[0.2em] text-muted">Budget disponible</p>
        ...
        {message && (
          <div className="mb-5 rounded-xl border border-carmine/20 bg-carmine/10 px-4 py-3 text-sm text-carmine-light">
            {message}
          </div>
        )}

        <section className="mb-5 grid gap-3 rounded-2xl border border-white/8 bg-pitch-900/70 p-4 md:grid-cols-[1fr_auto_auto]">
          <input
            value={query}
        ...
        <div className="mb-4 flex items-center justify-between">
          <p className="text-xs uppercase tracking-[0.16em] text-muted">{filteredListings.length} profils disponibles</p>
          <p className="hidden text-xs text-muted sm:block">Les notes reflètent le niveau actuel, le potentiel indique la marge de progression.</p>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {filteredListings.map((player) => {
            ...
            return (
              <article key={player.id} className="group overflow-hidden rounded-2xl border border-white/10 bg-pitch-900/90 shadow-lg shadow-black/10 transition hover:-translate-y-0.5 hover:border-carmine/30">
                <div className="flex items-start gap-4 border-b border-white/8 p-4">
                  <div className={`flex h-16 w-16 shrink-0 flex-col items-center justify-center rounded-2xl border ${getOverallStyle(player.overall)}`}>
                    <span className="font-display text-2xl font-semibold leading-none">{player.overall}</span>
                    <span className="mt-1 text-[9px] uppercase tracking-wider opacity-70">Note</span>
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <h2 className="truncate text-base font-semibold text-white">{player.first_name} {player.last_name}</h2>
                        <p className="mt-1 truncate text-xs text-muted">{player.clubs?.name ?? "Sans club"}</p>
                      </div>
                      <span className={`rounded-full border px-2.5 py-1 font-mono text-[10px] font-bold ${getPositionStyle(player.position)}`}>
                        {player.position}
                      </span>
                    </div>
                    <div className="mt-3 flex flex-wrap gap-2 text-[11px] text-zinc-300">
                      ...
                      <span className="rounded-md bg-white/5 px-2 py-1">Potentiel {player.potential}</span>
                      <span className="rounded-md bg-white/5 px-2 py-1">Forme {player.form}</span>
                      <span className="rounded-md bg-white/5 px-2 py-1">Moral {player.morale}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between gap-3 border-t border-white/8 bg-black/10 px-4 py-4">
                  <div>
